...
function CheckoutWizard({step, shoppingCart, triggerSubmit, submitForm, handleValidateFields, YupSchema, showOrderFailedModal, updateZip}) {
  ...
  useEffect(() => {
    if (shoppingCartAndDatesObj.length === 0) {
        ...
        const recentCart = shoppingCart.map(elem => ({'frecno': elem.frecno, 'itemNotes': elem.itemNotes, 'quantity': elem.quantity, 'itemUnitPriceOverride': _.get(elem,`itemUnitPriceOverride`,null), 'requestedShipDate': date}))
